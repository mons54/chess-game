module.exports = function(app, mongoose) {
	...
	app.get('/sponsorpay', function(req, res) {
		...
		if(url[1]) {
			...
			url = url[1].replace("sponsorpay?", "");
			
			var sid = get_param('sid', url);
			var amount = get_param('amount', url);
			var uid = get_param('uid', url);
			var transid = get_param('_trans_id_', url);
			
			if(sid && amount && uid && transid) {
				
				var sha1_of_important_data = sha1(security_token + uid + amount + transid); 
				...
				if (sid == sha1_of_important_data) {
					...
					sponsorpay.count({id:transid}, function (err, nb) {
						...
						var trans = new sponsorpay({ id : transid });
						trans.uid = uid;
						trans.amount = amount;
