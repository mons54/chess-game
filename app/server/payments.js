	...
	app.all('/payments', function(req, res) {
		...
		global.graph.post('/oauth/access_token?client_id=' + global.appId + '&client_secret=' + global.secret + '&grant_type=client_credentials', function (err, data) {
			...
			global.graph.get('/' + paymentId + '?access_token=' + data.access_token, function (err, data) {
				...
				payments.find({ id: data.id }, function (err, _data) {
					...
					if (err) {
						...
						users.find({ uid: data.user.id }, function (err, _data) {
							...
							users.update({ uid: data.user.id }, { $set: {tokens: token} }, function (err) { 
						...
						users.find({ uid: data.user.id }, function (err, _data) {
							...
							users.update({ uid: data.user.id }, { $set: { tokens: token } }, function (err) { 
								...
								payments.update({ id: data.id }, { $set: { type: 'refund', status: 'completed' } }, function (err) { 
