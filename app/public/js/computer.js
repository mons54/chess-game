...
(function ($) {
    ...
    $.widget("ui.computer", {
		...
		_init: function () {
			...
			that.computer = couleur1;
			
			that.joueur = couleur2;
			
			that.options[couleur1].name = $.options.lang[lang].player + ' 2';
			...
			if (!that.options[couleur2].name) {
				...
				that.options[couleur2].name = $.options.lang[lang].player + ' 1';
			}
			...
			if(that.jeu.tour == that.computer) {
        ...
        _charge: function (depart, arriver) {
			...
			else if(that.jeu.tour != that.computer){
        ...
        _resultat: function () {
            ...
            var game_over = $.options.lang[that.lang].game_over;
            var resultat = $.options.lang[that.lang][that.jeu.resultat.nom];

            $('<h2>' + game_over + '</h2>').appendTo(fenetre);
            $('<div class="resultat">' + resultat + '</div>').appendTo(fenetre);

            var winner = $.options.lang[that.lang].winner;
